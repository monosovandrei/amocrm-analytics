...
type Tab = 'workspace' | 'builder' | 'integration' | 'settings';
type UserRole = 'ADMIN' | 'ROP';
type SourceType = 'EVENT' | 'CURRENT';
type MetricType = 'count' | 'total_amount' | 'avg_amount' | 'conversion' | 'forecast' | 'contract';
type DisplayType = 'kpi' | 'funnel' | 'table' | 'forecast';
type WidgetSize = 'sm' | 'md' | 'lg';
type DenominatorType = 'previous' | 'first';
type ReportMode = 'contract' | 'single';
type ContractMetricType = 'created_deals' | 'stage_reached' | 'current_stage' | 'field_condition' | 'conversion';
type ContractMeasure = 'deal_count' | 'field_sum' | 'field_avg';
type ContractDisplay = 'number' | 'money' | 'percent';
type FieldOperator = 'equals' | 'contains' | 'is_set' | 'lt' | 'lte' | 'gt' | 'gte';
...
interface ContractConversionDraft {
  id: string;
  label: string;
  fromMetricId: string;
  toMetricId: string;
...
interface ContractDurationDraft {
  id: string;
  label: string;
  stageId: string;
}

interface ContractMetricPayload {
  id: string;
  label: string;
  type: ContractMetricType;
  measure?: ContractMeasure;
...
function buildQueryFromTemplate(template: ReportTemplate, workspaceFilters: ReportFilters) {
...
function buildConditionLabel(draft: ReportDraft, options: Options | null) {
  if (draft.mode === 'contract') {
...
function getStageName(options: Options | null, stageId: string) {
  if (!stageId) return '';
  return options?.pipelines.flatMap((pipeline) => pipeline.stages).find((stage) => stage.id === stageId)?.name ?? '';
}

function getFieldName(options: Options | null, fieldId: string) {
  if (!fieldId) return '';
  return options?.customFields.find((field) => field.externalId === fieldId)?.name ?? '';
}

function getMetric(template: Pick<ReportTemplate, 'name' | 'config'>, result: Record<string, any> | null) {
